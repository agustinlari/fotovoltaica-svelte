<script lang="ts">
  ...
  let form: Partial<Camion & { FechaDescargaUi?: string }> = {};
  ...
  async function save() {
    ...
    try {
      await apiPut(`/camiones/${state.selectedId}`, payload);
      state.info = 'Guardado';
      await load();
  ...
  <div class="content">
    ...
    {:else}
      <div class="form">
        ...
        <label>
          Conductor
          <input bind:value={form.NombreConductor} />
        </label>
        <label>
          DNI
        ...
        <label>
          Campa
          <input bind:value={form.UbicacionCampa} />
        </label>
        <label>
        ...
        <label>
          Contenedor
          <input bind:value={form.Container} />
        </label>
        <label>
          Albarán
